...
namespace SiteSandBox.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            UserProfile tmp = db.UserProfile.FirstOrDefault(u => u.UserName == name);
            if (tmp != null)
                return tmp.UserId;
            return -1;
        }
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.ChangeEmailSuccess ? "Votre email a été modifié."
                : null;
            ViewBag.HasLocalPassword = OAuthWebSecurity.HasLocalAccount(WebSecurity.GetUserId(User.Identity.Name));
            ViewBag.ReturnUrl = Url.Action("Manage");
            ViewBag.EMail = getEmailUser();
            return View();
        }
        ...
        private string getEmailUser()
        {
            ...
            UserProfile us = db.UserProfile.FirstOrDefault(u => u.UserName == User.Identity.Name);
            string tmp = "";
            if (us != null)
                tmp = us.Email;
            else
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Manage(LocalModel model)
        {
            bool hasLocalAccount = OAuthWebSecurity.HasLocalAccount(WebSecurity.GetUserId(User.Identity.Name));
            ViewBag.HasLocalPassword = hasLocalAccount;
            ViewBag.ReturnUrl = Url.Action("Manage");
            if (model.LocalEmail != null)
            {
                if (IsValid(model.LocalEmail.Email))
                {
                    UserProfile us = db.UserProfile.FirstOrDefault(u => u.UserName == User.Identity.Name);
                    if (us != null && us.Email != model.LocalEmail.Email)
                    {
                        us.Email = model.LocalEmail.Email;
                        db.Entry(us).State = EntityState.Modified;
                        ViewBag.Email = model.LocalEmail.Email;
                        db.SaveChanges();
                ...
                {
                    ModelState.AddModelError("LocalEmail.Email", "Le nouvel email n'est pas valide.");
                }
            }
            else if (hasLocalAccount)
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        changePasswordSucceeded = WebSecurity.ChangePassword(User.Identity.Name, model.LocalPassWord.OldPassword, model.LocalPassWord.NewPassword);
                    }
                    catch (Exception)
                    {
            ...
            else
            {
                ...
                ModelState state = ModelState["LocalPassWord.OldPassword"];
                if (state != null)
                {
                    state.Errors.Clear();
                }
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        WebSecurity.CreateAccount(User.Identity.Name, model.LocalPassWord.NewPassword);
                        ViewBag.Email = getEmailUser();
